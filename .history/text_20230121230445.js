...
function createItemList (userData, item) {
    ...
    for (let i = 0; i < itemList.length; i++) {
        ...
        let totalAmount = Math.floor(userData.money / itemList.price);
        let total = Math.floor(userData.money / itemList.price) * itemList.price;
        if (total > userData.money) {
            ...
            userData.increasePrice(itemList.increaseAssets(parseInt(total)));
            itemList.increasePurchaseQuantity(totalAmount);
        }
        })
    }
