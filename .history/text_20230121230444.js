...
function createItemList (userData, item) {
    ...
    for (let i = 0; i < itemList.length; i++) {
        ...
        let totalAmount = Math.floor(userData.money / itml.price);
        let total = Math.floor(userData.money / itml.price) * itml.price;
        if (total > userData.money) {
            alert("お金が足りません");
        } else if (itml.purchaseLimit == itml.purchaseQuantity) {
            alert("これ以上購入できません");
        } else {
            userData.reduceBalance(total);
            userData.increasePrice(itml.increaseAssets(parseInt(total)));
            itml.increasePurchaseQuantity(totalAmount);
        }
        })
    }
