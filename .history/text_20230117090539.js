...
function registerAccount () {
    ...
    else player = new UserAccount (userName, 20, 0, 30000, 0, 0, items, 0);

    displayNone(initialForm);
    displayBlock(mainPage);
    config.mainPage.innerHTML ="";
    config.mainPage.append(makeMainPage(player));
}
...
function loginAccount () {
    ...
    if (saveData == null) {alert("データが見つかりませんでした");
        ...
        let userData = new UserAccount(saveData["name"], saveData["age"], saveData["days"], saveData["money"], saveData["profitPerClick"], saveData["profitPerSeconds"], saveData["belongings"], saveData["hamburger"]);
    }
