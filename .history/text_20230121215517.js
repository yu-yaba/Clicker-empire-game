...
function loginAccount () {
    ...
    if (saveData == null) {alert("データが見つかりませんでした");
        ...
        for (let i = 0; i < saveData["itemList"].length; i++) {
            let eachItem = saveData["itemList"][i];
...
function createItemList (userData, items) {
    ...
    if (total > userData.money) {
        ...
        userData.increasePrice(items.increaseAssets(parseInt(total)));
        items.increasePurchaseQuantity(totalAmount);
    }
    });
    ...
    purchaseBtn.addEventListener("click", function () {
        if (itemList.price > userData.money) {
            alert("お金が足りません");
        } else if (item.purchaseLimit == items.purchaseQuantity) {
            alert("これ以上購入できません");
        } else {
            userData.reduceBalance(items.price);
            userData.increasePrice(items.increaseAssets(items.price));
