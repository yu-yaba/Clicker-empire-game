...
function createItemList (userData, itemList) {
    ...
    for (let i = 0; i < itemList.length; i++) {
        ...
        let total = Math.floor(userData.money / itemList[i].price) * itemList[i].price;
        if (total > userData.money) {
            ...
            userData.increasePrice(itemList[i].increaseAssets(parseInt(total)));
        ...
        purchaseBtn.addEventListener("click", function () {
            if (itemList.price > userData.money) {
                alert("お金が足りません");
            } else if (itemList.purchaseLimit == itemList.purchaseQuantity) {
                alert("これ以上購入できません");
            } else {
                userData.reduceBalance(itemList.price);
                userData.increasePrice(itemList.increaseAssets(itemList.price));
                itemList.increasePurchaseQuantity(1);
            }
