...
function createItemList (userData, item) {
    ...
    for (let i = 0; i < itemList.length; i++) {
        ...
        <div class="d-flex">
            ...
            <div>
                <button class='btn btn-info purchase-btn${i}'>× 1</button>
    ...
    let purchaseBtn = eachItemCon.querySelector(`.purchase-btn${i}`);
    purchaseBtn.addEventListener("click", function () {
        if (item[i].price > userData.money) {
            alert("お金が足りません");
        } else if (item[i].purchaseLimit == item[i].purchaseQuantity) {
            alert("これ以上購入できません");
        } else {
            userData.reduceBalance(item[i].price);
            userData.increasePrice(item.increaseAssets(item[i].price));
