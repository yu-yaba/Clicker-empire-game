...
function createItemList (userData, itemList) {
    ...
    for (let i = 0; i < itemList.length; i++) {
        ...
        purchaseBtn.addEventListener("click", function () {
            if (ite.price > userData.money) {
                alert("お金が足りません");
            } else if (ite.purchaseLimit == ite.purchaseQuantity) {
                alert("これ以上購入できません");
            } else {
                userData.reduceBalance(ite.price);
                userData.increasePrice(ite.increaseAssets(ite.price));
                ite.increasePurchaseQuantity(1);
            }
