...
class Item {
    constructor(name, imgUrl, price, profit, holdItem, purchaseLimit, type){
        this.name = name;
        this.imgUrl = imgUrl;
        this.price = price;
        this.profit = profit;
    ...
    increaseHoldItem (amount) {
        this.holdItem += amount;
    }
...
function loginAccount () {
    ...
    if (saveData == null) {alert("データが見つかりませんでした");
        ...
        for (let i = 0; i < saveData["items"].length; i++) {
            ...
            loginItems.push(new Item(eachItem["name"], eachItem["imgUrl"], eachItem["any"], eachItem["profit"], eachItem["holdItem"], eachItem["purchaseLimit"], eachItem["type"]))
        }
...
function createItemList (items, item, userData) {
    ...
    purchaseBtn.addEventListener("click", function () {
        if (parseInt(item.price) > userData.money) {
            alert("お金が足りません");
        } else if (item.purchaseLimit == item.holdItem) {
            alert("これ以上購入できません");
        } else {
